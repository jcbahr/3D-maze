...
def segmentType(seg):
    """Returns "horiz" if horizontal segment, "vert" if vertical segment.
...
def makeRay(eye, point):
    """Given two points, it returns a ray of the form ((x,y),(dx,dy)) where 
    the first point is (x,y) and (dx,dy) is the distance to the second point.
    It will crash if the two given points are identical."""
    (eyeX, eyeY) = eye
    (pX, pY) = point
    dx = float(pX - eyeX)
    dy = float(pY - eyeY)
...
def isHorizSeg(seg):
    return segmentType(seg) == "horiz"

def isVertSeg(seg):
    return segmentType(seg) == "vert"
...
def intersectRayAndVertSegment(ray, segment):
    """Given a ray and a "vert" segment, intersectRayAndSegment will return the
    point where the ray intersects the line generated by the segment.  If the
    segment is in the opposite direction of the ray, it returns None.  If the
    segment lies on the ray itself, it returns a "behind" keyword with the
    intersection.  If the ray and segment are parallel, it returns plus or
    minus infinity."""
    ((x1,y1), (x2,y2)) = segment
    (eye, (dx, dy)) = ray
    (eyeX, eyeY) = eye
...
def intersectRayAndHorizSegment(ray, segment):
    """Identical to intersectRayAndVertSegment but with a horizontal seg."""
    ((x1,y1), (x2,y2)) = segment
    (eye, (dx, dy)) = ray
    (eyeX, eyeY) = eye
    ySeg = float(y1) # = y2
...
def intersectRayAndRookSegment(ray, segment):
    """Given a ray and a segment, returns the of intersection or +/- infinity
    when parallel.  The point of intersection is not guaranteed to lie on the
    segment."""
    ((x1,y1), (x2,y2)) = segment
    if ((x1 != x2) and (y1 != y2)):
        assert(False), "not a rook segment"
...
def intersectWalls(wall, seg):
    """Given two perpendicular rook walls, returns the predicted
    intersection."""
...
def obstructSegViaSeg(eye, wall, seg):
    """This will generate obstructing points on seg when wall is blocking eye.
    It returns a list containing the segment and obstrutions.  We obstruct
    seg with wall."""
...
def chopHorizSegWithObstructions(segWithObstructions):
    """Given a "horiz" segment with obstructions, returns the remaining part of
    the segment which is visible."""
...
def chopVertSegWithObstructions(segWithObstructions):
    """Given a "vert" segment with obstructions, returns the remaining part of
    the segment which is visible."""
    (segment, obstructions) = segWithObstructions
    if (len(obstructions) == 0):
        return set([segment])
...
def chopSegWithObstructions(segWithObstructions):
    """Given a rook segment with obstructions, returns the remaining part of
    the segment which is visible.  It is assumed that the obstructions
    lie on the segment."""
    (segment, obstructions) = segWithObstructions
...
def obstructSegViaSetOfSegs(eyePoint, setOfSegs, segment):
    """Given an eye, a certain segment and a set of others, this returns the
    remaining visible portion of the specific segment when obstructed by the
    whole set."""
